...
class ChangeDetector:
    ...
    def __init__(self, **kwargs):
        ...
        self.train_metrics = {}
        self.difference_img_metrics = {"AUC": tf.keras.metrics.AUC()}   #计算曲线下面积
        self.change_map_metrics = {
            "ACC": tf.keras.metrics.Accuracy(),   #计算预测与标签匹配的频率
    ...
    def _difference_img(self, x, y, x_hat, y_hat):
        ...
        # d_x = _dense_gaussian_filtering(x, y, d_x)
        # d_y = _dense_gaussian_filtering(x, y, d_y)
        # pic(d_x, name="d_x", chanel="L")
        # pic(d_y, name="d_y", chanel="L")
    ...
    def _change_map_reverse(self, difference_img):
        """
            Input:
                difference_img - tensor of shape (h, w), (1, h, w)
                                 or (1, h, w, 1) in [0,1]
    ...
    def _change_pu(self, difference_img):
        """
            Input:
                difference_img - tensor of shape (h, w), (1, h, w)
                                 or (1, h, w, 1) in [0,1]
            Output:
                change_map - tensor with same shape as input, bool
        """
        min = tf.reduce_min(difference_img)
        max = tf.reduce_max(difference_img)
        print(f"difference_image min_val:{min}, max_val:{max}")

        return difference_img >= 0.1
        ...
        **kwargs,
        ...
        for epoch in trange(self.epoch.numpy() + 1, self.epoch.numpy() + epochs + 1):
            ...
            # if evaluation_dataset is not None:
                ...
                self.final_evaluate(evaluation_dataset=evaluation_dataset, save_images=True, final_filter="z_gaussian_filtered_diff")
    ...
    def evaluate(self, x, y, target_change_map, filter_=None):
        ...
        pic(difference_img, name="difference_img_preguass", chanel="L")
        if filter_ is not None:
            difference_img = _dense_gaussian_filtering(x, y, difference_img)
            self._ROC_curve(target_change_map, difference_img)
        pic(difference_img, name="difference_img", chanel="L")
        ...
        pic(change_map, name="change_map", chanel="L")
        tf.print("cohens kappa:", self.metrics_history["cohens kappa"][-1])
        tf.print("AUC:", self.metrics_history["AUC"][-1])

        confusion_map = self._confusion_map(target_change_map, change_map)
        pic(confusion_map, name="confusion_map", chanel="RGB")
    ...
    def final_evaluate(self, evaluation_dataset, save_images, final_filter, **kwargs):
        ...
        for eval_data in evaluation_dataset.batch(1):
            # print(eval_data)
            # print(evaluation_dataset.batch(1))
